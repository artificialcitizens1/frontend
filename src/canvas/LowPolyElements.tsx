...
const RenderWindowDetails: React.FC<{
  ...
  const baseY =
    config.baseYOffset !== undefined ? config.baseYOffset : config.height / 2 + config.vSpacing;
  ...
  for (let floor = 0; floor < config.numFloors; floor++) {
    ...
    config.sides.forEach((side) => {
      const totalWidthForWindows =
        config.countPerFloorPerSide * config.width +
        Math.max(0, config.countPerFloorPerSide - 1) * config.hSpacing;
        ...
        switch (side) {
          ...
          case "back":
            ...
            position = [
              windowX,
              currentFloorCenterY,
              (side === "front" ? parentDepth / 2 : -parentDepth / 2) +
                (side === "front" ? windowDepth / 2 : -windowDepth / 2),
            ];
            break;
          case "left":
          case "right":
            ...
            position = [
              (side === "left" ? -parentWidth / 2 : parentWidth / 2) +
                (side === "left" ? -windowDepth / 2 : windowDepth / 2),
              currentFloorCenterY,
              windowX,
            ];
            rotation[1] = Math.PI / 2;
    ...
    parts: [
      ...
      },
      {
        ...
        door: {
          ...
          positionY: 0.1,
          offsetX: 5,
        }, // offsetX on Z axis for side door
        windows: {
          sides: ["front", "back", "left", "right"],
          countPerFloorPerSide: 2,
          numFloors: 2,
  ...
  return (
    <>
      {buildingClusters.map((cluster) => (
        <group key={cluster.id} position={cluster.basePosition} rotation={cluster.baseRotation}>
          {cluster.parts.map((part) => {
            ...
            return (
              <group key={part.id} position={finalPositionOffset} rotation={partRotation}>
                ...
                {/* Render Details */}
                {part.door && (
                  <RenderDoorDetails config={part.door} parentArgs={args} parentShape={shape} />
                )}
                {part.windows && (
                  <RenderWindowDetails
                    config={part.windows}
                ...
                {part.floorLines && (
                  <RenderFloorLineDetails
                    config={part.floorLines}
                    parentArgs={args}
                    parentShape={shape}
                  />
                )}
