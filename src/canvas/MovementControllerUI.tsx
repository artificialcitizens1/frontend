...
const inputStyle = "shadow appearance-none border rounded w-16 py-1 px-2 text-gray-700 leading-tight focus:outline-none focus:shadow-outline text-xs";
...
const PersonInputGroup = ({ personId, coords, onChange }: PersonInputGroupProps): JSX.Element => (
  <div className="mb-2 p-2 border border-gray-300 rounded">
    <h4 className="font-bold text-sm mb-1">{personId === 'person1' ? 'Person 1 Target' : 'Person 2 Target'}</h4>
    <div className="flex items-center mb-1">
      <label htmlFor={`${personId}-x`} className={labelStyle}>X:</label>
      <input type="number" id={`${personId}-x`} value={coords.x} onChange={(e) => onChange(personId, 'x', e.target.value)} className={inputStyle} placeholder="X" />
    </div>
    <div className="flex items-center mb-1">
      <label htmlFor={`${personId}-y`} className={labelStyle}>Y:</label>
      <input type="number" id={`${personId}-y`} value={coords.y} onChange={(e) => onChange(personId, 'y', e.target.value)} className={inputStyle} placeholder="Y (Ground)" />
    </div>
    <div className="flex items-center">
      <label htmlFor={`${personId}-z`} className={labelStyle}>Z:</label>
      <input type="number" id={`${personId}-z`} value={coords.z} onChange={(e) => onChange(personId, 'z', e.target.value)} className={inputStyle} placeholder="Z" />
    </div>
  </div>
);
